...
#define PELIB_MAPPING_OUT_WARMUP "x [*,*]\n"
#define PELIB_MAPPING_OUT_NODE_COLUMN_WIDTH 6
#define PELIB_MAPPING_OUT_PROC_COLUMN_WIDTH 4
#define PELIB_MAPPING_OUT_AFFECT " :="
#define PELIB_MAPPING_OUT_ENDING ";"
struct mapping
{
...
int
pelib_mapping_insert_task(mapping_t*, processor_id, task_t*);
/// Removes a task from this mapping
int
pelib_mapping_remove_task(mapping_t*, task_id);
/// Finds the index of processor in the mapping processor array, from the processor's ID
int
pelib_mapping_find_processor_index(mapping_t*, processor_id);
/// Finds the processor (its processor_id) which handles the task pointed by the task_id parameter
processor_id
pelib_mapping_find_processor(mapping_t*, task_id);
/// Inserts a new processor in the mapping.
int
pelib_mapping_insert_processor(mapping_t*, processor_t*);
/// Not implemented. Removes a processor from the mapping.
int
pelib_mapping_remove_processor(mapping_t*, int);

int
pelib_mapping_violations(mapping_t*);

char*
pelib_mapping_drawstr(mapping_t*, char*);
/// Draws the mapping to a .fig file in the appropriate format
int
pelib_mapping_draw(mapping_t*, FILE*);
...
mapping_t*
pelib_mapping_loadfile(mapping_t* mapping, FILE* ilp_mapping_file);
/// Reads a mapping from a string in the same format than produced by ilp solver
mapping_t*
pelib_mapping_loadstr(mapping_t*, char*);

/// Reads a mapping from a string similarly to pelib_mapping_readfile(), and filter tasks according to filter function
/// \param mapping Mapping that will point to the data read
...
/// Similar to pelib_mapping_readfile, but filter tasks according to a given user filter function
/// \param mapping Mapping that will point to the data read
/// \param ilp_mapping_file File from where to read the mapping calculated by the ilp solver
/// \param filter Pointer to a user function that returns 0 if the task should be kept, non zero otherwise.
mapping_t*
pelib_mapping_loadfilterfile(mapping_t* mapping, FILE* ilp_mapping_file, int
( filter)(task_t*));
